    ...
    public ISkin getSkin(IPlayerProfile profile)
    {
        ...
        Shared.pool.execute(() -> {
            ...
            if (!Shared.isOfflinePlayer(profile.getPlayerID(), profile.getPlayerName()))
                data = CachedDownloader.create().setLocal(_dirU, uuid.toString()).setRemote("%s/skins/%s", _host, uuid).setDataStore(Shared.store).setProxy(MinecraftUtils.getProxy()).read();
            if (data == null && !Shared.isBlank(name))
                data = CachedDownloader.create().setLocal(_dirN, name).setRemote("%s/skins/%s", _host, name).setDataStore(Shared.store).setProxy(MinecraftUtils.getProxy()).read();
            if (data != null)
